...
from telegram_youtube_downloader.download_thread import DownloadThread
from telegram_youtube_downloader.youtube_searcher import YoutubeSearcher
...
from telegram_youtube_downloader.decorators.telegram_bot_error_handler import (
	TelegramBotErrorHandler,
)
from telegram_youtube_downloader.decorators.telegram_bot_command_interceptor import (
	TelegramBotCommandInterceptor,
)
	...
	def start(self):
				...
				keyboard.append(
					[
						InlineKeyboardButton(
							TelegramBot.__video_title_formatter(
		...
		async def search_video_selection_menu_handler(
			...
			keyboard = [
				[
					InlineKeyboardButton(
						f"audio - Max ({self.downloader.get_max_audio_duration()})",
						callback_data="{{audio}}",
					),
					InlineKeyboardButton(
						f"video - Max ({self.downloader.get_max_video_duration()})",
						callback_data="{{video}}",
					),
				]
			]
			await query.edit_message_text(
				text="Download this video as", reply_markup=InlineKeyboardMarkup(keyboard)
			)

		@TelegramBotErrorHandler.menu_handler()
		async def search_download_type_menu_handler(
			update: Update, context: ContextTypes.DEFAULT_TYPE
		) -> None:
			"""
